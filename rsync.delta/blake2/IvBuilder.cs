...
namespace Rsync.Delta.Blake2
{
    internal static class IvBuilder
    {
        private static readonly Blake2BTreeConfig SequentialTreeConfig = 
            new Blake2BTreeConfig() { IntermediateHashSize = 0, LeafSize = 0, FanOut = 1, MaxHeight = 1 };
		...
		public static ulong[] ConfigB(Blake2BConfig config, Blake2BTreeConfig treeConfig)
		{
			...
			if (config.Salt != null)
			{
				...
				rawConfig[4] = Core.BytesToUInt64(config.Salt, 0);
				rawConfig[5] = Core.BytesToUInt64(config.Salt, 8);
			}
			// Personalization
			if (config.Personalization != null)
			{
				...
				rawConfig[6] = Core.BytesToUInt64(config.Personalization, 0);
				rawConfig[7] = Core.BytesToUInt64(config.Personalization, 8);
			}
