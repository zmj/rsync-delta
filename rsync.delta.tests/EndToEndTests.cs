...
namespace Rsync.Delta.Tests
{
    public class EndToEndTests
    {
        ...
        public async ValueTask Test(string v1, string v2, int? blockLength = null, int? strongHashLength = null)
        {
            string actual = await Do(v1, v2, Options(blockLength, strongHashLength));
            Assert.Equal(v2, actual);
        }

        private async ValueTask<string> Do(string v1, string v2, SignatureOptions options)
        {
            var utf8 = Encoding.UTF8;
            return utf8.GetString(await Do(utf8.GetBytes(v1), utf8.GetBytes(v2), options));
        }

        private async ValueTask<byte[]> Do(byte[] v1, byte[] v2, SignatureOptions options)
        {
            var signature = new MemoryStream();
            await _rsync.GenerateSignature(new MemoryStream(v1), signature, options);
            signature.Seek(0, SeekOrigin.Begin);

            var delta = new MemoryStream();
            await _rsync.GenerateDelta(signature, new MemoryStream(v2), delta);
            delta.Seek(0, SeekOrigin.Begin);

            var patched = new MemoryStream();
            await _rsync.Patch(delta, new MemoryStream(v1), patched);
